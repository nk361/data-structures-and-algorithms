...
c_avl_binary_tree<DataType, NodeType, Operation>::c_avl_binary_tree(DataType const& val) : c_binary_tree<DataType, NodeType, Operation>(val) {}

template<class DataType, class NodeType, typename Operation>
c_avl_binary_tree<DataType, NodeType, Operation>::c_avl_binary_tree(std::vector<DataType> const& vals) : c_binary_tree<DataType, NodeType, Operation>(vals[0])
{
	for (size_t i{ 1 }; i < vals.size(); ++i)
		c_avl_binary_tree<DataType, NodeType, Operation>::add_item(vals[i]);
}

template<class DataType, class NodeType, typename Operation>
void c_avl_binary_tree<DataType, NodeType, Operation>::add_item(DataType const& val)
{
	Operation const op = Operation();

	NodeType * * current{ &(c_avl_binary_tree<DataType, NodeType, Operation>::root) };//pointer to pointer so that pointer references can change originals
...
template<class DataType, class NodeType, typename Operation>
void c_avl_binary_tree<DataType, NodeType, Operation>::add_items(std::vector<DataType> const& vals)
{
	for (DataType val : vals)
		c_avl_binary_tree<DataType, NodeType, Operation>::add_item(val);
}

template<class DataType, class NodeType, typename Operation>
void c_avl_binary_tree<DataType, NodeType, Operation>::remove_item(DataType const& val)
{
	std::vector<NodeType * *> ancestors{ c_avl_binary_tree<DataType, NodeType, Operation>::find_node_with_path(&(c_avl_binary_tree<DataType, NodeType, Operation>::root), val) };
...
template<class DataType, class NodeType, typename Operation>
size_t c_avl_binary_tree<DataType, NodeType, Operation>::height(NodeType * r_avl_node)
{
	if (r_avl_node == nullptr)
		return 0;

	size_t max_height = 1;
	NodeType * current{ r_avl_node };
	std::vector<std::pair<NodeType *, bool>> ancestors;//store the previous nodes and the direction it went from it
...
template<class DataType, class NodeType, typename Operation>
void c_avl_binary_tree<DataType, NodeType, Operation>::rebalance(NodeType * * chain_node)//call this with a reference to the grandparent
{
	if (static_cast<int>(height((*chain_node)->children[0]) - height((*chain_node)->children[1])) > 1)//violation on left
		if (height((*chain_node)->children[0]->children[0]) > height((*chain_node)->children[0]->children[1]))//right rotation needed
			*chain_node = c_avl_binary_tree<DataType, NodeType, Operation>::rotate_right((*chain_node));
		else//left right rotation needed
			*chain_node = c_avl_binary_tree<DataType, NodeType, Operation>::rotate_left_right((*chain_node));
		...
		if (height((*chain_node)->children[1]->children[0]) < height((*chain_node)->children[1]->children[1]))//left rotation needed
			*chain_node = c_avl_binary_tree<DataType, NodeType, Operation>::rotate_left((*chain_node));
		else//right left rotation needed
			*chain_node = c_avl_binary_tree<DataType, NodeType, Operation>::rotate_right_left((*chain_node));
}