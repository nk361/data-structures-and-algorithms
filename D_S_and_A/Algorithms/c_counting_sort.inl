...
std::vector<MyType>& c_counting_sort<MyType, Operation>::sort(std::vector<MyType>& arr)
{
	std::map<MyType, MyType, Operation> index_and_count;//map instead of unordered to keep indexes in order like an array
	...
	for (typename std::map<MyType, MyType, Operation>::iterator it{ std::next(index_and_count.begin(), 1) }; it != index_and_count.end(); ++it)//start AFTER first spot, go to last item
		it->second += std::prev(it, 1)->second;//adds count from item before the current

	for (typename std::map<MyType, MyType, Operation>::iterator it{ index_and_count.begin() }; it != index_and_count.end(); ++it)
