...
std::vector<DataType>& c_counting_sort<DataType, Operation>::sort(std::vector<DataType>& arr)
{
	std::map<DataType, DataType, Operation> index_and_count;//map instead of unordered to keep indexes in order like an array
	for (int i{ 0 }; i < static_cast<int>(arr.size()); ++i)
	{
		typename std::map<DataType, DataType, Operation>::iterator found{ index_and_count.find(arr[i]) };
	...
	for (typename std::map<DataType, DataType, Operation>::iterator it{ std::next(index_and_count.begin(), 1) }; it != index_and_count.end(); ++it)//start AFTER first spot, go to last item
		it->second += std::prev(it, 1)->second;//adds count from item before the current

	for (typename std::map<DataType, DataType, Operation>::iterator it{ index_and_count.begin() }; it != index_and_count.end(); ++it)
		for (int i{ static_cast<int>(it->second - (it == index_and_count.begin() ? 0 : std::prev(it, 1)->second)) }; i > 0; --i)//start at the difference between current and previous for occorances
			arr[it->second - i] = it->first;//set arr at index occurrences - how many placed equal to the current value
	return arr;
}